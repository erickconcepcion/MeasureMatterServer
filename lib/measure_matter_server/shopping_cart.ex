defmodule MeasureMatterServer.ShoppingCart do
  @moduledoc """
  The ShoppingCart context.
  """

  import Ecto.Query, warn: false
  alias MeasureMatterServer.Repo
  alias MeasureMatterServer.Catalog
  alias MeasureMatterServer.ShoppingCart.Cart
  ...
  alias MeasureMatterServer.ShoppingCart.CartItem
  ...
  def add_item_to_cart(%Cart{} = cart, %Catalog.Product{} = product) do
    if from(ci in CartItem, where: ci.product_id==^product.id and ci.cart_id==^cart.id)
      |> Repo.exists?() do
      {1, [item]}=
        from(ci in CartItem, where: ci.product_id==^product.id and ci.cart_id==^cart.id,select: [:quantity])
